...
export function GlassCard({ tint = 0, className, children, ...props }: GlassCardProps) {
  return (
    <GlassSurface
      tint={tint}
      radius={20}
...
export function GlassCardDescription({ className, ...props }: ComponentProps<"p">) {
  return <p className={cn("text-sm text-slate-500 dark:text-white/55", className)} {...props} />;
}

export function GlassCardContent({ className, ...props }: ComponentProps<"div">) {
  return <div className={cn("text-sm text-slate-600 dark:text-white/75", className)} {...props} />;
}

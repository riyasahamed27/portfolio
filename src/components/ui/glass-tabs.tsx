...
import { GlassSurface, useGlassDark } from "@/components/ui/glass";
...
export function TabsList({
  ...
  useEffect(() => {
    const paint = () => {
      ...
      el.style.transform = `translateX(${x}px) scale(${1 + 0.5 * q}, ${
        1 - 0.28 * q
      })`;
    };
    const subs = [
  ...
  return (
    <GlassSurface
      tint={tint}
      radius={9999}
      className={cn("relative inline-flex px-2 py-1", className)}
      {...props}
    >
      <div
        ref={pill}
        ...
        style={{
          ...
          background: dark
            ...
            : "rgba(0,0,0,0.08)",
          boxShadow: dark
            ? "inset 0 1px 0 rgba(255,255,255,0.3), 0 2px 10px rgba(0,0,0,0.18)"
            : "inset 0 1px 0 rgba(255,255,255,0.85), 0 2px 10px rgba(0,0,0,0.1)",
        }}
      />
